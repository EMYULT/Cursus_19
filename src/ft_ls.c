...
// Il faudrait verifier que le path est valide pour return 1 ou -1
...
void	handle_arg(b_arg *arg)
{
	...
	mylist = NULL;
	if (arg->with_arg == 0)
		no_param(dir, d, mylist, arg);
	else
	{
		if (arg->is_l == 1)
			param_l(dir, d, fs, mylist, arg);
		if (arg->is_R == 1)
			param_R(dir, d, fs, mylist, arg);
		if (arg->is_r == 1)
			param_r(dir, d, fs, mylist, arg);
		if (arg->is_t == 1)
			param_t(dir, d, fs, mylist, arg);
		if (arg->is_a == 1)
			param_a(dir, d, fs, mylist, arg);
	}
}
